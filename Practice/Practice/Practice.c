 ...
 ***/
enum {
    CST_CELL, CST_ROW, CST_COL, CST_BOX, CST_N
};
...
static int search(struct sudansu *ctx, unsigned k) {
    ...
    for (r = c->down; r != c; r = r->down)            /* step 3: travel the sudoku board via Dancing Link						*/
    {
        ...
        for (j = r->right; j != r; j = j->right)    /*    3-2: covering														*/
            cover(j->column);
        if (search(ctx, k +
                        1))                        /*    3-3: recursive call(then find next value which holds sudoku rule	*/
...
static int parse_grid(struct sudansu *ctx, FILE *stream) {
    ...
    for (j = 0; j < N; j++) {
        ...
        for (i = 0;
             i < N; i++)                /* setting the solution board which is covered by all initial conditions */
        {
